...
public class TestAccountBalance {
	...
	public void testAccountSingleThreadSameCcyTransfer() throws CustomException {
		...
		final AccountInfoDAO accountDAO = h2DaoFactory.getAccountInfoDAO();
		...
		AccountInfo accountFrom = accountDAO.getAccountById(3);

		AccountInfo accountTo = accountDAO.getAccountById(4);
	...
	public void testAccountMultiThreadedTransfer() throws InterruptedException, CustomException {
		...
		AccountInfo accountFrom = accountDAO.getAccountById(1);

		AccountInfo accountTo = accountDAO.getAccountById(2);
	...
	public void testTransferFailOnDBLock() throws CustomException, SQLException {
		...
		try {
			...
			h2DaoFactory.getAccountInfoDAO().transferAccountBalance(transaction);
			conn.commit();
		} catch (Exception e) {
		...
		assertTrue(h2DaoFactory.getAccountInfoDAO().getAccountById(6).getBalance().equals(originalBalance));
		assertTrue(h2DaoFactory.getAccountInfoDAO().getAccountById(5).getBalance().equals(originalBalance));
	}
