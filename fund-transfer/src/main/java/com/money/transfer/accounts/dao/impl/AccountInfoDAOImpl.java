package com.money.transfer.accounts.dao.impl;

import com.money.transfer.accounts.dao.AccountInfoDAO;
import com.money.transfer.accounts.dao.H2DAOFactory;
...
public class AccountInfoDAOImpl implements AccountInfoDAO {
	...
	public List<AccountInfo> getAllAccounts() throws CustomException {
		...
		ResultSet rs = null;
		List<AccountInfo> allAccounts = new ArrayList<AccountInfo>();
		try {
			conn = H2DAOFactory.getConnection();
			stmt = conn.prepareStatement(SQL_GET_ALL_ACC);
			rs = stmt.executeQuery();
			while (rs.next()) {
				AccountInfo acc = new AccountInfo(rs.getLong("AccountId"), rs.getString("UserName"),
	...
	public AccountInfo getAccountById(long accountId) throws CustomException {
		Connection conn = null;
		PreparedStatement stmt = null;
		ResultSet rs = null;
		...
		try {
			...
			rs = stmt.executeQuery();
			if (rs.next()) {
				acc = new AccountInfo(rs.getLong("AccountId"), rs.getString("UserName"), rs.getBigDecimal("Balance"),
	...
	public long createAccount(AccountInfo account) throws CustomException {
		Connection conn = null;
		PreparedStatement stmt = null;
	...
	public int updateAccountBalance(long accountId, BigDecimal deltaAmount) throws CustomException {
		...
		try {
			...
			if (rs.next()) {
				targetAccount = new AccountInfo(rs.getLong("AccountId"), rs.getString("UserName"),
						rs.getBigDecimal("Balance"), rs.getString("CurrencyCode"));
				if (log.isDebugEnabled())
	...
	public int transferAccountBalance(UserTransaction userTransaction) throws CustomException {
		...
		try {
			...
			rs = lockStmt.executeQuery();
			if (rs.next()) {
				fromAccount = new AccountInfo(rs.getLong("AccountId"), rs.getString("UserName"),
						rs.getBigDecimal("Balance"), rs.getString("CurrencyCode"));
				if (log.isDebugEnabled())
			...
			rs = lockStmt.executeQuery();
			if (rs.next()) {
				toAccount = new AccountInfo(rs.getLong("AccountId"), rs.getString("UserName"), rs.getBigDecimal("Balance"),
						rs.getString("CurrencyCode"));
				if (log.isDebugEnabled())
